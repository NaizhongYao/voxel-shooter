...
/**
 * 关卡 03「废弃电台」的敌人出生点池（20 人，tier 分层与 01/02 同构）。
 *
 * 十字形四翼夹一个中庭：没有单一主动线，玩家从北侧庭院进正门，
 * 但四个凹角都能绕进侧翼，所以「先打哪一翼」完全是玩家的选择——
 * 伏击者优先蹲在两个方向都可能被撞见的门内侧，巡逻者覆盖每间有
 * 净空环路的房间，中庭本身留给武装巡逻者与冲锋手当纵深威胁。
 *
 * ══ 数量与 tier 结构与 01/02 完全一致 ══
 *   tier 1（简单 13 人）：九间房 + 中庭都有人，保证「找得到、打得过」。
 *   tier 2（困难 17 人）：中庭追加武装巡逻、发射机房与主机房补人。
 *   tier 3（专家 20 人）：宿舍出现 DMR 武装巡逻（隔中庭长视线），
 *                        演播厅追加第二个伏击者 + 第二个冲锋手。
 * 全难度 +30%（原 10/13/15）靠追加蹲守者/伏击者做到，不新增巡逻路线。
 *
 * ══ 冲锋手：电台这关的招牌敌人（中庭，tier 1）══
 *
 * 脆但极快：确认交火后不再原地磨蹭，全速冲到霰弹有效距离才停下开火，
 * 破坏「找柱子打远处 AR/DMR」的舒适区。见 `ARCHETYPE.RUSHER` 与
 * `config.js` 的 `RUSHER_ENEMY`。三档都会遇到，专家档在演播厅再加一个。
 *
 * ══ 巡逻路线全部由求解器算出，不是手写 ══
 *
...
export const ENEMY_SPAWNS03 = [
  ...
  /**
   * ── 冲锋手（电台正式引入，中庭 tier 1）──
   *
   * 中庭是十字开阔地，原本的玩法是「找柱子和墙垛打远处的 AR/DMR」，
   * 冲锋手专门破坏这种舒适的远距离对枪：一旦真正确认交火（举枪反应
   * 结束之后），他不再原地磨蹭，而是全速冲向玩家，冲到霰弹有效距离
   * 才停下开火，逼玩家不能一直站在原地瞄同一个方向。
   *
   * 站在中庭中央——足够开阔，冲刺路径不会被墙垛截断。
   * tier 1：三档都会遇到，是电台这关的招牌敌人（呼应诊所的盾兵）。
   */
  // (36.5,29.5) 是中庭东侧墙垛外的真实净空；原 (32.5,29.5) 落在墙垛里，
  // 会被 Enemy.escapeIfStuck 悄悄挪开，表面可达但冲刺起点不稳定。
  { x: 36.5, y: Y1, z: 29.5, yaw: 0, archetype: ARCHETYPE.RUSHER, weapon: 'shotgun', tier: 1 },

  /**
   * ── 人数 +30%（全难度）：13 / 17 / 20 ──
   *
   * 与 01/02 同一套做法：全部用伏击者补数量，不用蹲守者、不新增巡逻路线
   * （原地不动的蹲守者会整局瞪墙，见 test/combat.test.mjs 那条断言）。
   */
  // tier 1 追加：发射机房、演播厅两个伏击者
  { x: 52.5, y: Y1, z: 20.5, yaw: Math.PI, archetype: ARCHETYPE.AMBUSHER, weapon: 'pistol', tier: 1 },
  { x: 36.5, y: Y1, z: 14.5, yaw: Math.PI, archetype: ARCHETYPE.AMBUSHER, weapon: 'smg', tier: 1 },

  // tier 2 追加：主机房伏击者
  { x: 38.5, y: Y1, z: 44.5, yaw: -Math.PI / 2, archetype: ARCHETYPE.AMBUSHER, weapon: 'ar', tier: 2 },

  /**
   * ── 专家档追加：演播厅冲锋手（tier 3）──
   *
   * 与「专家档追加第二个伏击者」同一个模式：专家档要同时应付
   * 远处蹲守的 DMR 精英（导播室）和两个方向冲过来的近战威胁
   * （中庭 + 这里），逼玩家不能把所有注意力放在一个方向上。
   */
  { x: 38.5, y: Y1, z: 11.5, yaw: Math.PI, archetype: ARCHETYPE.RUSHER, weapon: 'shotgun', tier: 3 },
];
